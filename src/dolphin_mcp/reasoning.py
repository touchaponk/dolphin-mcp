...
Rules:
 ...
 - You can import library whatever you want
 - You can call tools using the available tool functions when appropriate
 """


def get_feedback_system_prompt(question: str, guidelines: str, is_initial_feedback: bool = True) -> str:
    """
...
class MultiStepReasoner:
    ...
    async def generate_plan(self, question: str, guidelines: str, 
                          generate_func, model_cfg: Dict, all_functions: List[Dict]) -> str:
        """
        ...
        planning_conversation = [
            {
                "role": "system", 
                "content": get_feedback_system_prompt(question, guidelines, is_initial_feedback=True)
            },
            {
                "role": "user",
...
Available tools: {json.dumps([f["name"] for f in all_functions], indent=2)}"""
            }
        ]
    ...
    async def execute_reasoning_loop(self, question: str, guidelines: str, initial_plan: str,
        ...
        for i in range(self.config.max_iterations):
            logger.info(f"Reasoning iteration {i + 1}/{self.config.max_iterations}")
            ...
            try:
                ...
                logger.debug(f"Assistant response: {assistant_text[:200]}...")
                ...
                if code_blocks and self.config.enable_code_execution:
                    for code in code_blocks:
                        ...
                        logger.debug(f"Code output: {output[:200]}...")
                ...
                if not code_blocks and not tool_calls:
                    logger.warning("No code execution or tool calls detected, might be stuck")
                    # Continue anyway, the model might be thinking
                
            except Exception as e:
                logger.error(f"Error in reasoning iteration {i + 1}: {str(e)}")
                return False, f"Error during reasoning: {str(e)}"
        
        # If we reach here, we've hit max iterations without a final answer
        logger.warning(f"Reached max iterations ({self.config.max_iterations}) without final answer")
